...
export const roomNameSchema = yup.object({
  roomName: yup.string().test(function (val) {
    if (val && !val.split(' ').length) return this.createError({ message: 'Spaces are not allowed' });
    return true;
  }),
});
...
export const RoomNameForm: React.FC<Props> = ({ open, handleClose, youtubeUrl }) => {
  ...
  return (
    <Dialog open={open} onClose={handleClose} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description">
      ...
      <DialogContent>
        <DialogContentText id="alert-dialog-description">Give your room a unique name!</DialogContentText>
        <Controller
